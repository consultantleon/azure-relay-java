...
class TokenRenewer {
    ...
    protected TokenRenewer(HybridConnectionListener listener, String appliesTo,
            Duration tokenValidFor) {
    ...
    private CompletableFuture<SecurityToken> getTokenAsync(boolean raiseTokenRenewedEvent) {
        ...
        return this.listener
                ...
                .thenApply((token) -> {
                    RelayLogger.logEvent("getTokenStop", this.listener,
                            token.getExpiresAtUtc().toString());
    ...
    private void scheduleRenewTimer(SecurityToken token) {
        if (scheduleLock.tryLock()) {
            try {
                ...
                interval =
                        interval.compareTo(RelayConstants.CLIENT_MINIMUM_TOKEN_REFRESH_INTERVAL) < 0
                        ? RelayConstants.CLIENT_MINIMUM_TOKEN_REFRESH_INTERVAL : interval;

                if (this.renewTimerTask != null) {
                    // There may already be renew task scheduled previously, cancel to prevent
                    // replication
